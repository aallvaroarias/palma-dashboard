...
export default function Gerencial() {
  ...
  const fechaCorteLabel = r.fecha_corte
    ? new Date(r.fecha_corte + 'T12:00:00').toLocaleDateString('es-PA', { day: 'numeric', month: 'long', year: 'numeric' })
    : null;
    ...
    <div className="animate-fade-in">
      ...
      {fechaCorteLabel && (
        <div className="flex items-center gap-2 mb-4 px-3 py-2 rounded-lg"
          style={{ background: 'rgba(26,127,166,0.08)', border: '1px solid rgba(45,200,216,0.2)', display: 'inline-flex' }}>
          <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: 'var(--cyan)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <span style={{ fontSize: '11px', color: 'var(--muted)' }}>
            Corte de ventas: <strong style={{ color: 'var(--cyan)' }}>{fechaCorteLabel}</strong>
          </span>
        </div>
      )}
