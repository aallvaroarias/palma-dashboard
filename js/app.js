...
async function loadAll(){
  ...
  if (cobertura) STATE.cobertura = cobertura.filter(r => !esBodega(r.vendedor));
  if (cobNegocio) STATE.cobNegocio = cobNegocio.filter(r => !esBodega(r.vendedor));
...
function renderGerencial(){
  ...
  if (!r) return;
  $('kVenta').textContent = fmt(r.venta_real);
  $('kDevol').textContent = fmt(r.devolucion_total);
  $('kDevolPct').innerHTML = `<span class="down">${pct(r.pct_devolucion)}</span> de venta bruta`;
  $('kNeta').textContent = fmt(r.venta_neta);
  $('kCob').textContent = pct(r.cobertura_pct);
  $('kCobSub').textContent = `${r.clientes_impactados} / ${r.clientes_maestro}`;
  $('kCobBar').style.width = Math.min(r.cobertura_pct,100) + '%';
...
function renderCobertura(){
  ...
  const cobField = data[0] ? Object.keys(data[0]).find(k => k.includes('cobertura')) : 'cobertura_';
  $('tbCobertura').innerHTML = data.map(r => `
    <tr>
      ...
      <td style="text-align:right">${pct(r[cobField])}</td>
  ...
  $('tbCero').innerHTML = STATE.clientesCero.por_vendedor.map(r => `
    <tr><td>${r.vendedor}</td><td style="text-align:right">${r.cantidad}</td></tr>
  `).join('');
}
