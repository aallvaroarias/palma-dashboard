...
META_DN_CREMAS_PCT = 10.0    # v6.3 — 10% del universo del vendedor
META_NUEVOS           = 10    # era 15 — CORREGIDO
META_CERO_PCT         = 4.0   # era 2%  — CORREGIDO
META_DEVOLUCION_PCT   = 5.0   # ≤5% → $22, >5% → $0

# SKUs de Zuko (negocio 10-TMLUC, marca 295-Zuko) — desde endpoint skus
ZUKO_SKUS = {
    '1059746', '1060754', '1060755', '1062885', '1062886',
    '1065480', '1065481', '1065482', '1065484', '1065485',
    '1065487', '1065490', '1065491', '1065492', '1065493',
    '1065494', '1065495', '1065502', '1065503', '1092046',
}
...
def calc_dn_cremas(detalle_data, fallback_universo=0):
    """Cuenta clientes únicos con venta positiva en referencias CREMAS del negocio Galletas."""
    filas = detalle_data.get("detalle_filas", [])
    clientes = set()
    skus_vistos = {}  # cod_sku -> producto
    for row in filas:
        if "CREMA" not in str(row.get("producto", "")).upper():
            continue
        ...
        if clt:
            clientes.add(clt)
        cod_sku = str(row.get("cod_sku", ""))
        if cod_sku:
            skus_vistos[cod_sku] = str(row.get("producto", ""))
    n = len(clientes)
    ...
    skus_list = [{"cod_sku": k, "producto": v} for k, v in sorted(skus_vistos.items())]
    return n, uni, pct, skus_list
...
def fetch_all_cremas(vendedores_list):
    ...
    for v in sorted(vendedores_list, key=lambda x: x["cod"]):
        ...
        try:
            ...
            n, uni, pct, skus_list = calc_dn_cremas(data, mae)
            cremas_por_vend[cod] = {"n": n, "universo": uni, "pct": pct, "skus": skus_list}
            print(f" {n}/{uni} ({pct:.1f}%) — {elapsed}s")
        except Exception as e:
            print(f" ERROR: {e}")
            cremas_por_vend[cod] = {"n": 0, "universo": mae, "pct": 0.0, "skus": []}
    return cremas_por_vend


def calc_zuko_from_detalle(detalle_data, fallback_universo=0):
    """Calcula venta y clientes únicos Zuko filtrando ZUKO_SKUS en Bebidas TMLUC."""
    filas = detalle_data.get("detalle_filas", [])
    clientes = set()
    venta_total = 0.0
    skus_detail = {}
    for row in filas:
        cod_sku = str(row.get("cod_sku", ""))
        if cod_sku not in ZUKO_SKUS:
            continue
        cant = float(row.get("cant_neta", 0))
        venta = float(row.get("valor_venta", 0))
        if cant <= 0:
            continue
        venta_total += max(0.0, venta)
        clt = str(row.get("cod_cliente", ""))
        if clt:
            clientes.add(clt)
        if cod_sku not in skus_detail:
            skus_detail[cod_sku] = {
                "producto": str(row.get("producto", "")),
                "venta": 0.0, "clientes": set(),
            }
        skus_detail[cod_sku]["venta"] += max(0.0, venta)
        if clt:
            skus_detail[cod_sku]["clientes"].add(clt)
    n = len(clientes)
    uni = int((detalle_data.get("resumen") or {}).get("universo", fallback_universo))
    if uni <= 0:
        uni = int(fallback_universo)
    pct = round(n / uni * 100, 1) if uni > 0 else 0.0
    skus_list = [
        {"cod_sku": k, "producto": v["producto"],
         "venta": round(v["venta"], 2), "n_clientes": len(v["clientes"])}
        for k, v in sorted(skus_detail.items())
    ]
    return round(venta_total, 2), n, uni, pct, skus_list


def fetch_all_zuko(vendedores_list):
    """Fetches detalle_cobertura_negocio Bebidas TMLUC por vendedor y calcula venta Zuko."""
    print("  → Obteniendo venta Zuko (detalle_cobertura_negocio Bebidas TMLUC ×14)...")
    zuko_por_vend = {}
    for v in sorted(vendedores_list, key=lambda x: x["cod"]):
        cod = str(v["cod"]).zfill(3)
        mae = int(v.get("maestro", 0))
        ...
        try:
            ...
            data = fetch("detalle_cobertura_negocio",
                         {"cod_asesor": cod, "negocio": "Bebidas TMLUC"})
            elapsed = (datetime.now() - t0).seconds
            venta, n, uni, pct, skus_list = calc_zuko_from_detalle(data, mae)
            zuko_por_vend[cod] = {
                "venta": venta, "n_clientes": n, "universo": uni, "pct": pct,
                "skus": skus_list,
            }
            print(f" {fmt_usd(venta)} / {n} cli ({pct:.1f}%) [{len(skus_list)} SKUs] — {elapsed}s")
        except Exception as e:
            print(f" ERROR: {e}")
            zuko_por_vend[cod] = {
                "venta": 0.0, "n_clientes": 0, "universo": mae, "pct": 0.0, "skus": [],
            }
    return zuko_por_vend
...
def calcular_incentivos(vendedores_list, inc_map, horeca_por_vend,
                        ...
                        cuotas_por_vend, cremas_por_vend, zuko_por_vend):
    results = []
    for v in sorted(vendedores_list, key=lambda x: x["cod"]):
        cod  = str(v["cod"])
        inc  = inc_map.get(cod, {})
        ...
        # ── DN Galletas Cremas (meta 10% universo v6.3) ───────────────────
        ...
        cremas_skus_det = cremas_data.get("skus", [])
        cremas_resultado = f"{fmt_pct(pct_cremas)} ({n_cremas}/{uni_cremas})"
        p_cremas = VBI_DN_GALLETAS_CREMAS if pct_cremas >= META_DN_CREMAS_PCT else 0.0
        ...
        # ── Lista de indicadores completa (v6.1) ─────────────────────────
        indicadores = [
            # CALIDAD
            ...
            ind("DN Galletas Cremas ×6", "calidad",
                ...
                cremas_resultado,
                f"≥{META_DN_CREMAS_PCT:.0f}% maestra",
                p_cremas,
                nota="calculado con referencias CREMAS del negocio Galletas"),
        ...
        results.append({
            ...
            # DN Cremas (indicador calculado v6.3)
            "n_cremas": n_cremas, "uni_cremas": uni_cremas, "pct_cremas": pct_cremas,
            "p_cremas": p_cremas, "cremas_skus_det": cremas_skus_det,
        })
    return results
...
def build_pdf_consolidado(results, styles, out):
    ...
    #          | Devol | Fotos* | Ej.Choc | Ej.Gall | Ej.Beb.TMLUC | NE Ind
    ...
    rows.append([
        ...
        Paragraph(f"<b>{fmt_usd(tot['p_cremas'])}</b>",  styles["cons_num"]),
...
def write_csv_dn_galletas_cremas(results, out):
    with open(out,"w",newline="",encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["cod_vendedor","vendedor","universo","clientes_cremas",
                    "dn_cremas_pct","meta_pct","cumple","vbi","pago",
                    "productos_skus_considerados","observacion"])
        for r in results:
            cumple = "SI" if r["pct_cremas"] >= META_DN_CREMAS_PCT else "NO"
            skus_str = "; ".join(
                f"{s['cod_sku']}:{s['producto']}" for s in r["cremas_skus_det"]
            ) if r["cremas_skus_det"] else "ninguno"
            w.writerow([
                r["cod"], r["nombre_limpio"],
                r["uni_cremas"], r["n_cremas"],
                f"{r['pct_cremas']:.1f}",
                f"{META_DN_CREMAS_PCT:.0f}",
                cumple,
                f"{VBI_DN_GALLETAS_CREMAS:.0f}",
                f"{r['p_cremas']:.2f}",
                skus_str,
                "Calculado — referencias CREMAS negocio Galletas / meta 10% universo",
            ])
...
def write_csv_zuko(results, out):
    ...
    with open(out, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["cod_vendedor", "vendedor", "sku", "producto", "marca", "negocio",
                    "venta_neta_zuko", "clientes_impactados_zuko", "meta_zuko",
                    "cumplimiento_zuko", "vbi", "pago", "observacion"])
        obs_calc = ("Venta calculada desde VMXC (SKUs 295-Zuko, Bebidas TMLUC); "
                    "pago pendiente confirmación meta Zuko")
        obs_sin   = "Sin venta Zuko en el período (SKUs 295-Zuko, Bebidas TMLUC)"
        for r in results:
            skus = r.get("zuko_skus_det", [])
            if skus:
                for s in skus:
                    w.writerow([
                        r["cod"], r["nombre_limpio"],
...
def main():
    ...
    print(f"  PALUMAR S.A. — v6.3 (DN Cremas + Bebidas TMLUC liquidados)")
    print(f"{'='*65}\n")
    ...
    # 3. Venta Zuko (requiere 14 llamadas a detalle_cobertura_negocio Bebidas TMLUC)
    print("\n3. Obteniendo venta Zuko (puede tomar ~3-6 minutos)...\n")
    zuko_por_vend = fetch_all_zuko(vendedores_data)

    # 4. Calcular
    print("\n4. Calculando incentivos (v6.3 — DN Cremas + Bebidas TMLUC liquidados)...\n")
    ...
    # 5. PDFs individuales
    print("\n5. Generando PDFs individuales (3 páginas c/u)...\n")
    styles = make_styles()
    for r in results:
    ...
    total_cremas = sum(r["p_cremas"] for r in results)
    ...
    n_cremas_cumple = sum(1 for r in results if r["pct_cremas"] >= META_DN_CREMAS_PCT)
    ...
    print(f"   ✓ DN Galletas Cremas: {n_cremas_cumple}/{len(results)} vendedores pagan "
          f"(meta ≥{META_DN_CREMAS_PCT:.0f}%) → {fmt_usd(total_cremas)}")
    ...
    print(f"   ℹ {n_pend_uniq} indicador(es) aún pendiente(s) (sin fuente — no suman al total)")

    # 9. Resumen final
    TOTAL_V62 = 1340.80
    diff_v = grand - TOTAL_V62
    print(f"\n{'='*65}")
    print(f"  RESUMEN FINAL — TOTAL A PAGAR JUNIO 2026  (v6.3)")
    print(f"{'='*65}")
    ...
    print(f"  DN Galletas Cremas (VBI $33):       {fmt_usd(total_cremas)}")
    ...
    print(f"  ─────────────────────────────────────────────")
    print(f"  vs v6.2 (era {fmt_usd(TOTAL_V62)}):            {'+' if diff_v>=0 else ''}{fmt_usd(diff_v)}")
